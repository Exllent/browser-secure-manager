...
class AppSettingsDialog(QDialog):
    ...
    def _build_general_page(self) -> QWidget:
        ...
        self.profile_cache_days_combo.setFixedWidth(140)
        self.export_all_logs_button = QPushButton(_("Export all logs"))
        self.export_error_logs_button = QPushButton(_("Export error logs"))
        for button in (self.export_all_logs_button, self.export_error_logs_button):
            button.setFixedWidth(240)
        layout.addWidget(self.confirm_before_delete_check)
        layout.addWidget(self.profile_cache_check)
        profile_cache_days_row = QHBoxLayout()
        profile_cache_days_row.setContentsMargins(0, 0, 0, 0)
        profile_cache_days_row.addWidget(self.profile_cache_days_combo)
        profile_cache_days_row.addStretch(1)
        profile_cache_form = QFormLayout()
        profile_cache_form.addRow(_("Profile cache retention"), profile_cache_days_row)
        layout.addLayout(profile_cache_form)
        logs_export_layout = QHBoxLayout()
        logs_export_layout.addWidget(self.export_all_logs_button)
        logs_export_layout.addWidget(self.export_error_logs_button)
        logs_export_layout.addStretch(1)
        layout.addLayout(logs_export_layout)
        layout.addStretch(1)
